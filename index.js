...
//variable created for the profits as an array
var profits = []
for (var i = 0; i < finances.length - 1; i++) {
    // Total change in profits/losses from month to month and the dates pushed to a new array
    profits.push([finances[i + 1][0], finances[i + 1][1] - finances[i][1]]);
}
//Created new variables for the Greatest Increase and Decrease in profits and a variable for the sum of the profits
var greatestIncrease = ["", 0]
var greatestDecrease = ["", 0]

var sumProfits = 0
//for and if loop to calculate greatest increase and decrease
for (var i = 0; i < profits.length; i++) {
    sumProfits += profits[i][1];
    //if profits its greater than the greatest increase then
    if (greatestIncrease[1] < profits[i][1]) {
        //= greatest increase
        greatestIncrease = profits[i]
    }
    //if profits is less than greatest decrease then
    if (greatestDecrease[1] > profits[i][1]) {
        //= greatest decrease
        greatestDecrease = profits[i]
    }
}
